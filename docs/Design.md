...
GET    /api/auth/*                 — Better Auth endpoints (public)

GET    /api/stories                — List user's stories (owner)
POST   /api/stories                — Create story (auth)
GET    /api/stories/:id            — Get story (owner)
DELETE /api/stories/:id            — Soft delete (owner)
GET    /api/stories/discover       — Browse public stories (public)
GET    /api/stories/:id/scenes     — Get all scenes (owner)
GET    /api/stories/:id/codex      — Get codex entries (owner)
GET    /api/stories/:id/echoes     — Get echoes (owner)
POST   /api/stories/:id/continue   — Continue story (owner)
POST   /api/stories/:id/fork       — Fork story (owner OR public+forkable)

GET    /api/vault                  — List vault characters (owner)
POST   /api/vault                  — Create vault character (auth)
GET    /api/vault/:id              — Get vault character (owner)
PATCH  /api/vault/:id              — Update vault character (owner)
DELETE /api/vault/:id              — Delete vault character (owner)
POST   /api/vault/from-protagonist/:id — Save protagonist to vault (auth)
```
...
Using **Better Auth** with Drizzle adapter. Auth tables (`user`, `session`, `account`, `verification`) are in `packages/database/src/auth-schema.ts`.

### Access Model (Like GitHub Repos)

Once follows a GitHub-like access model:

1. **Private by default** — All story data is private. Only the owner can see scenes, codex, echoes, protagonists, deferred characters.

2. **Public stories for discovery** — Stories with `visibility: "public"` appear on `/discover`. Others can see metadata (title, description, upvotes) and fork.

3. **Fork creates ownership** — When user B forks user A's public story, user B now owns the fork. They have full access to their copy, but no access to A's original private data.

### Authorization Layers

| Layer            | Where               | Purpose                                             |
| ---------------- | ------------------- | --------------------------------------------------- |
| `authMiddleware` | Global (`index.ts`) | Extracts session/user from headers, sets in context |
| `requireAuth`    | Per-route           | Returns 401 if no user                              |
| Owner check      | In-handler          | Verifies `resource.userId === user.id`              |

### Route Access Patterns

| Pattern               | Routes                                                           |
| --------------------- | ---------------------------------------------------------------- |
| **Public**            | `/health`, `/api/auth/*`, `/api/stories/discover`                |
| **Auth only**         | `POST /api/stories`, `POST /api/vault`                           |
| **Owner only**        | All other story/vault routes                                     |
| **Owner OR forkable** | `POST /api/stories/:id/fork` (others need public + allowForking) |

### Middleware Files

- `apps/api/src/lib/auth.ts` — Better Auth config with Drizzle adapter
- `apps/api/src/middleware/auth.ts` — `authMiddleware` and `requireAuth`
...
1. **Get user from context** — Use `c.get("user")` in handlers. Check ownership with `resource.userId === user.id`.
...
1. **Social auth providers** — GitHub, Google sign-in
2. **Retry logic** — LLM calls don't retry on failure
3. **Timeout handling** — No timeout on LLM calls
4. **Rate limiting** — Not implemented
5. **Caching** — Not implemented
6. **Real streaming** — Using fake stream (full response, then send word-by-word)

## What IS Implemented ✅

1. **Authentication** — Better Auth with email/password, Drizzle adapter
2. **Authorization** — Owner-only access for all story/vault data
3. **Echo evaluation loop** — Checked each turn, triggered echoes woven into narration
4. **Codex extraction** — Runs in background after each scene
5. **Deferred character triggers** — Evaluated each turn, introduced when conditions met
6. **Memory system** — Mem0 + Qdrant + Neo4j integrated
7. **SSE streaming** — Fake stream endpoint works
8. **Fork endpoint** — With visibility + allowForking permission check
9. **Discover endpoint** — Lists public forkable stories
10. **Protagonist snapshots** — Saved on each scene for forking
11. **All CRUD endpoints** — Stories, scenes, codex, echoes, protagonists, deferred, vault

---

## Recently Completed (2024-12-27)

### Session 1: Core LLM Integration
- ✅ OpenAI Responses API integration with structured output
- ✅ System, initialize, continue prompts
- ✅ Story creation with LLM opening scene
- ✅ Story continuation with protagonist updates

### Session 2: Full Feature Implementation
- ✅ **Echo System** — Evaluation, planting, resolution
- ✅ **Codex Extraction** — Background entity extraction
- ✅ **Memory System** — Mem0 with Qdrant vectors + Neo4j graph
- ✅ **SSE Streaming** — Fake stream (word-by-word delivery)
- ✅ **Fork Endpoint** — With `allowForking` permission
- ✅ **Discover Endpoint** — Public forkable stories
- ✅ **Deferred Characters** — Vault characters with trigger conditions
- ✅ **Protagonist Snapshots** — Saved each scene for forking
- ✅ **Services Refactor** — Clean separation: llm, story, echo, protagonist, codex, memory, deferred

### Architecture Highlights
- All services in `apps/api/src/services/`
- All prompts in `apps/api/src/prompts/`
- Routes are thin orchestrators
- Docker compose for local dev (Qdrant + Neo4j)
